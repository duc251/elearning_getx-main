...
class ExerciseStepController extends GetxController with WidgetsBindingObserver {
  ...
  final stepController = ScrollController();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      pageController.animateToPage(pageIndex.value, duration: Duration(milliseconds: 300), curve: Curves.linear);
    }
  }
  ...
  prevQuestion() {
    ...
    pageIndex.value -= 1;
    pageController.animateToPage(pageIndex.value, duration: Duration(milliseconds: 300), curve: Curves.linear);
    var stepOffset = 56.0 * pageIndex.value;
    if (stepOffset < 56 * 3) {
      stepOffset = 0;
    } else {
      stepOffset -= 56 * 3;
    }
    stepController.animateTo(stepOffset, duration: Duration(milliseconds: 300), curve: Curves.linear);
    update();
  }

  nextQuestion() {
    if (pageIndex.value == (userExerciseResponse.userExercise?.questions?.length ?? 0) - 1) {
      return;
    }
    pageIndex.value += 1;
    pageController.animateToPage(pageIndex.value, duration: Duration(milliseconds: 300), curve: Curves.linear);
  ...
  void updateProcess(Questions q) {
    ...
    update();
    // if (question?.qDisplayContent?.type != QuestionTypes.multipleChoice && question?.qDisplayContent?.type != QuestionTypes.speaking) {
    //   nextQuestion();
    // }
  }
  ...
  void onRework() async {
    Get.close(1);
    // final userExerciseId = userExerciseResponse.userExercise?.id ?? 0;
    // final response = await ExerciseProvider.instance.continueUserExercise("$userExerciseId");
    // if (response.error.isEmpty) {
    //   Get.close(2);
    ...
    //       'userExerciseResponse': response.data,
    //       'exercisableType': userExerciseResponse.userExercise?.exercisableType ?? '',
    //       'exercisableId': userExerciseResponse.userExercise?.exercisableId ?? '',
    //     });
    //   });
    // } else {
    //   showDialog(
